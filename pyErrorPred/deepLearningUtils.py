...
class dataloader:
    ...
    def __init__(self,
        ...
        temp = []
        #np.random.seed(7)
        print("IM USING THE NEW VERSION")
        for p in proteins:
            # check datadir
            ...
            if not os.path.exists(join(path,"native.features.npz")):
                continue
            
            # Loading data by where it came from.
            if not multi_dir:
                path = join(self.datadir, p)
                samples = [join(path, f) for f in listdir(path) if isfile(join(path, f)) and "features.npz" in f]
            else:
                samples = []
