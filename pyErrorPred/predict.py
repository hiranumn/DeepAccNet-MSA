...
def predict(samples, distogram, modelpath, outfolder, verbose=False, transpose=False, ensemble=False):
    if ensemble:
        n_models = 5
    else:
        n_models = 2
...
def predict_separate_distogram(samples, distograms, modelpath, outfolder, verbose=False, transpose=False, ensemble=False):
    if ensemble:
        n_models = 5
    else:
        n_models = 2
    for i in range(1, n_models):
        ...
        for j in range(len(samples)):
            try:
                if verbose: print("Predicting for", samples[j], "(XD network rep"+str(i)+")") 
                tmp = join(outfolder, samples[j]+".features.npz")
                batch = getData(tmp, outfolder, np.load(distograms[j])['dist'].astype(np.float32))
                lddt, estogram, mask = model.predict2(batch)
                if transpose:
                    estogram = (estogram + np.transpose(estogram, [1,0,2]))/2
                    mask = (mask + mask.T)/2
...
def merge(samples, outfolder, verbose=False):
    for j in range(len(samples)):
        try:
            ...
            print("failed to merge:", samples[j])
                
def clean(samples, outfolder, noEnsemble=False, verbose=False):
        ...
        try:
            ...
            if not noEnsemble:
                for j in range(1,5):
                    ...
                    if isfile(join(outfolder, samples[i]+".rep"+str(j)+".npz")):
                        os.remove(join(outfolder, samples[i]+".rep"+str(j)+".npz"))
        except:
            print("Failed to clean:", samples[j])